                            ...
                            <div class="bg-white rounded-lg p-3 border border-gray-200 hover:shadow-sm transition-shadow h-auto flex flex-col">
                                <div class="flex flex-col items-center mb-3 flex-grow">
                                    ...
                                    <div class="text-center">
                                        @php
                                            $productName = $product['product_name'] ?? ($product['product']->productName ?? 'Unknown Product');
                                            // Prefer real SKU from product if available
                                            $sku = $product['product']->productSKU ?? null;
                                            if (!$sku && !empty($productName)) {
                                                // Fallback: try to parse trailing (SKU)
                                                if (preg_match('/\(([^)]+)\)$/', $productName, $m)) {
                                                    $sku = $m[1];
